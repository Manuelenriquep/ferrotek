...
def generar_portafolio(tipo="master"):
    ...
    if tipo in ["master", "casas"]:
        ...
        pdf.multi_cell(0, 6, "A. SERIE TRADICIONAL: Techo a dos aguas (PVC Colonial), aleros amplios y estética clásica.\nB. SERIE M (MODERNA): Eficiencia minimalista. Techos ocultos y espacios abiertos.")
        pdf.ln(5)
...
if st.session_state.view == 'home':
    ...
    with c2: st.download_button("🏠 Brochure Casas", generar_portafolio("casas"), "Ferrotek_Casas.pdf", "application/pdf", use_container_width=True)
...
# --- CASAS (TABS) ---
    ...
    with tab_trad:
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("#### Estilo Clásico")
            mod_t = st.selectbox("Modelo:", ["T-1: Refugio (30m2)", "T-2: Familiar (48m2)", "T-3: Hacienda (70m2)"])
            ...
            if st.text_input("Cliente Tradicional:", key="cli_t"):
                st.download_button("PDF", generar_pdf_cotizacion(st.session_state.get('cli_t'), "Casa Tradicional", data_t, f"{mod_t}"), "cot_trad.pdf")
        with c2:
    ...
    with tab_mod:
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("#### Estilo Minimalista")
            mod_m = st.selectbox("Modelo:", ["M-2: Minimalista (45m2)", "M-3: Familiar Pro (70m2)"])
            ...
            if st.text_input("Cliente Serie M:", key="cli_m"):
                st.download_button("PDF", generar_pdf_cotizacion(st.session_state.get('cli_m'), "Casa Serie M", data_m, f"{mod_m}"), "cot_mod.pdf")
        with c2:
            try: st.image("vivienda_suite.png", caption="Fachada Serie M", use_container_width=True)
...
# --- MUROS ---
    ...
    with c1:
        ...
        if st.text_input("Cliente:"): 
            st.download_button("PDF", generar_pdf_cotizacion("Cli", "Muro", data, f"Muro {tipo} {ml}x{alt}m"), "cot.pdf")
    with c2: 
...
# --- DOMOS ---
    ...
    with c1:
        ...
        if st.text_input("Cliente:"): 
            st.download_button("PDF", generar_pdf_cotizacion("Cli", "Domo", data, f"Domo {ancho}x{fondo}m"), "cot.pdf")
    with c2: 
...
# --- AGUA ---
    ...
    with c1:
        ...
        if st.text_input("Cliente:"): 
            st.download_button("PDF", generar_pdf_cotizacion("Cli", "Tanque", data, f"Tanque {vol}L"), "cot.pdf")
